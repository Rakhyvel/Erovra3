...
struct terrain* Terrain_Create(int mapSize, float* map, float* trees, SDL_Texture* texture)
{
    ...
    int status = 0;
    retval->ore = Noise_Generate(retval->tileSize, 4, (unsigned)time(0), &status);
    retval->timber = trees;
    ...
    Terrain_FindContinentsAndOceans(retval);
    return retval;
}
...
SDL_Color Terrain_RealisticColor(float* map, int mapSize, int x, int y, float i)
{
    ...
    if (mapSize >= 512.0f && Terrain_IsBorder(map, mapSize, mapSize, x, y, 0.5f, 1)) {
        ...
        i *= 2;
        i = powf(i, 91);
        return Terrain_HSVtoRGB(200 - 20 * i - light, 0.90 - 0.7 * i - light, 0.3 + 0.6 * i + light);
    } else {
        // ground
        i = (i - 0.5f) * 2;
        i = powf(i, 0.5);
        return Terrain_HSVtoRGB(47 + 80 * i - light, 0.18 + (i * 0.25) - light, .75 - i * 0.4 + light);
    }
}
...
void Terrain_Update(struct terrain* terrain)
{
    ...
    if (terrain_zoom < 1.5 * Apricot_Height / 1024.0f) {
        terrain_zoom = 1.5 * Apricot_Height / 1024.0f;
    }
...
static void addAdjacentOcean(struct terrain* terrain, int x, int y, Arraylist** queue)
{
    Vector newSquare = { x, y };
...
static void oceanFloodFill(struct terrain* terrain, Vector square)
{
    Arraylist* queue = Arraylist_Create(10, sizeof(Vector));
    Arraylist_Add(&queue, &square);
    ...
    while (queue->size > 0) {
        square = *(Vector*)Arraylist_Get(queue, 0);
        Arraylist_Remove(queue, 0);
        addAdjacentOcean(terrain, square.x + 1, square.y, &queue);
        addAdjacentOcean(terrain, square.x - 1, square.y, &queue);
        addAdjacentOcean(terrain, square.x, square.y + 1, &queue);
        addAdjacentOcean(terrain, square.x, square.y - 1, &queue);
    }
    Arraylist_Destroy(queue);
}
